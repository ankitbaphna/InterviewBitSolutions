...
public class Bracketology {
    ...
    private void bracketPremu (String output, int openCount, int closeCount, int total) {
        if(openCount == total && closeCount == total) {
            System.out.println(output);
        } else {
            if (openCount < total) {
                bracketPremu(output+"(", openCount+1, closeCount, total);
            }
            // if(closeCount < total) {  --- this doesn't work..At anytime close should not be greater than open
            // or else it's not valid sequence
            if(closeCount < openCount) {
                bracketPremu(output + ")", openCount, closeCount+1, total);
            }
        }
    }
    ...
    public static void main(String args[]){
        ...
        char result[] = new char[n*2];
         //matcher.bracketPermutation(result, 3, 0, 0, 0);

        matcher.bracketPremu("", 0, 0, 3);
